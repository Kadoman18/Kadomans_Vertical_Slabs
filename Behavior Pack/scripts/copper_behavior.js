...
// NOT WORKING ~~~~~~~
function damageAxe(item, player) {
  if (!item || !axeIds.includes(item.typeId)) return;
  const playerInventory = player.getComponent('inventory');
  const slot = playerInventory.container.getSlot(player.selectedSlotIndex);
  ...
  if (damageRoll < damageChance) {
    ...
    const newAxe = item.clone(); // SHIT
    newAxe.durability = durability.damage - 1; // FUCKIN
    slot.setItem(newAxe); // SUCKS
  } else {
    console.warn('LUCKY');
  }
